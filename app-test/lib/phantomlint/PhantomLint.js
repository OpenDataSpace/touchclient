            ...
            for (x; x < list.length; x++) {
                ...
                if (filesystem.isFile(currPath + list[x])) {
                    if (this.exclusions) {
                        ...
                        for (j; j < this.exclusions.length; j++) {
                            if (currPath + list[x] === this.exclusions[j]) { exclude = true; }
                        }
    ...
    lintFiles : function() {
        ...
        for (var appName in this.files) {
            for (var j in this.files[appName]) {
                ...
                if (!result) {
                    errorList.push("\t<file name=\"" + filesystem.absolute(file) + "\">");
                    for  (i; i < totalErrors; i++)  {
                        issue = JSLINT.errors[i];

                        if (issue) {
                            errorList.push("\t\t<issue line=\"" + issue.line + "\" char=\"" +
                                     issue.character + "\" reason=\"" + this.encode(issue.reason) +
                                     "\" evidence=\"" + this.encode(issue.evidence) + "\" />");
